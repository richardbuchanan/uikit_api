 ...
 * @{
 * @lead get_familiar Get familiar with the basic setup and structure of UIkit 8.x-2.x.
 * UIkit 8.x-2.x does not come with the required UIkit framework files
 * because, in general,
 ...
 * instructions below to get started with using UIkit 8.x-2.x.
 *
 * @divider
 *
 * @section download_uikit Download UIkit
 * First of all you need to download UIkit 8.x-2.x. There are three ways to
 * do this:
 ...
 * article before installing UIkit 8.x-2.x. We only provide the download
 * methods below, not how to install themes.
 *
 * @heading h3 NOTE: Recommended for theme developers only @endheading
 * Since UIkit 8.x-2.x is still in development, none of the following ways to
 * download UIkit 8.x-2.x are recommended for use on production sites. UIkit
 * 8.x-2.x is still considered unstable.
 *
 ...
 * following command to download UIkit 8.x-2.x-dev with Drush.
 *
 * @inlineblockcode drush dl uikit-8.x-2.x-dev @endinlineblockcode
 *
 ...
 * There are no requirements to use UIkit 8.x-2.x.
 * @}
 */
 ...
 * @{
 * @lead subtheme Create a custom theme by inheriting the UIkit 8.x-2.x base theme.
 ...
 * @{
 * @lead implementations Functions and templates for the user interface to be implemented by UIkit 8.x-2.x.
 * Drupal's default template renderer is a simple PHP parsing engine that
 * includes the template and stores the output. The default template engine in
 * Drupal 8 is Twig. This is the template engine utilized by UIkit 8.x-2.x.
 *
