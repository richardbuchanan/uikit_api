...
(function ($) {
  ...
  Drupal.CTools.bindCollapsible = function () {
    ...
    if (content.length) {
      ...
      var toggle = $('<i class="docs-ctools-toggle uk-text-center"></i>');
      ...
      if ($container.hasClass('ctools-collapsed')) {
        toggle.toggleClass('uk-icon-caret-right');
        content.hide();
      }
      ...
      var clickMe = function () {
        ...
        toggle.toggleClass('uk-icon-caret-right');
        toggle.toggleClass('uk-icon-caret-down');

        // If we're supposed to remember the state of this class, do so.
        if ($container.hasClass('ctools-collapsible-remember') && $container.attr('id')) {
          var state = toggle.hasClass('uk-icon-caret-right') ? -1 : 1;
