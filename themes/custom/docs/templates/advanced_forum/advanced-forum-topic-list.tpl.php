...
<table id="forum-topic-<?php print $topic_id; ?>" class="forum-table forum-table-topics uk-table uk-table-hover uk-table-striped uk-table-condensed uk-table-middle">
  <thead>
  ...
  <?php foreach ($topics as $topic): ?>
    ...
    <?php if ($topic->new && $topic->new_replies > 0) : ?>
      <?php $topic_new = ' <span class="uk-text-danger">' . t('updated') . '</span>'; ?>
    <?php elseif ($topic->new && $topic->new_replies <= 0) : ?>
      <?php $topic_new = ' <span class="uk-text-danger">' . t('new') . '</span>'; ?>
    <?php endif; ?>

    <tr class="<?php print $topic->zebra;?> <?php print $topic->sticky_class;?>">
      <td class="forum-topic-icon uk-position-relative"><div class="forum-icon"><?php print $topic->icon; ?></div></td>
